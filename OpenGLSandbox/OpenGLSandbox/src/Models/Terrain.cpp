...
namespace fsi
{
    ...
    Terrain::Terrain(TechniqueCache* manager)
        : m_wireframe(0)
        , m_triSize(8.f)
    {
    ...
    void Terrain::init()
    {
        ...
        int fw, fh, fcomp;
        unsigned char* fimg = stbi_load("res/perlinHmap.png", &fw, &fh, &fcomp, STBI_rgb_alpha);

        int dw, dh, dcomp;
        unsigned char* dimg = stbi_load("res/lunar.png", &dw, &dh, &dcomp, STBI_rgb_alpha);

        int nw, nh, ncomp;
        unsigned char* nimg = stbi_load("res/noise.png", &nw, &nh, &ncomp, STBI_rgb_alpha);
        ...
        /*heightMap = glGetUniformLocation(m_program->getProgramId(), "heightMap");
        fineMap = glGetUniformLocation(m_program->getProgramId(), "detailHeightMap");
        detailMap = glGetUniformLocation(m_program->getProgramId(), "terrainTexture");
        noiseMap = glGetUniformLocation(m_program->getProgramId(), "noiseMap");
        detMap = glGetUniformLocation(m_program->getProgramId(), "patchTexture");*/

        /*m_program->setUniformAttribute("heightMap", heightTexUnit);
        m_program->setUniformAttribute("detailHeightMap", fineTexUnit);
        m_program->setUniformAttribute("terrainTexture", dirtTexUnit);
        m_program->setUniformAttribute("noiseMap", noiseTexUnit);
        m_program->setUniformAttribute(detMap, detTexUnit);
*/
        ...
        m_program->setUniformAttribute("detail", detail);
    ...
    void Terrain::useDetail(bool detailEnabled)
    {
