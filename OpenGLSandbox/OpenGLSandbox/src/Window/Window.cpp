...
namespace fsi
{
    Window::Window(VideoMode videoMode, const std::string& title, Window::Style style)
        : m_size(videoMode.getWidth(), videoMode.getHeight())
        , m_prevSize(videoMode.getWidth(), videoMode.getHeight())
        ...
        , m_isResizing(false)
    {
        ...
        int left = (GetDeviceCaps(screenDC, HORZRES) - videoMode.getWidth()) / 2;
        int top = (GetDeviceCaps(screenDC, VERTRES) - videoMode.getHeight()) / 2;
        int width = videoMode.getWidth();
        int height = videoMode.getHeight();
        ReleaseDC(nullptr, screenDC);

        // Encode the windows style
        DWORD win32Style = translateWindowStyle(style);
        ...
        SetWindowPos(m_windowHandle, HWND_TOP, left, top, width, height, SWP_SHOWWINDOW);
    ...
    void Window::processMessage(UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_SIZE:
        {
            // Consider only events triggered by a maximize or an un-maximize
    ...
    DWORD Window::translateWindowStyle(Window::Style style)
    {
        ...
        {
            if (style & Style::TitleBar)      win32Style |= WS_CAPTION;
            if (style & Style::Resizable)     win32Style |= WS_THICKFRAME | WS_MAXIMIZEBOX | WS_MINIMIZEBOX;
            if (style & Style::CloseControls) win32Style |= WS_SYSMENU;
            if (style & Style::Overlapped)    win32Style |= WS_OVERLAPPED;
            if (style & Style::Popup)         win32Style |= WS_POPUP;
        }
